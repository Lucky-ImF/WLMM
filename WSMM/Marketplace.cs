...
namespace WSMM
{
    public partial class Marketplace : Form
    {
        ...
        private void CreateModEntry(string ModString)
        {
            ...
            Size PanelSize = new Size((int)Math.Floor(ModFlow_Panel.Width * 0.32), (int)Math.Floor(ModFlow_Panel.Height * 0.51));
            Mod_Panel[EntryID] = new Panel();
            Mod_Panel[EntryID].Size = PanelSize;
            ...
            int IconSize = (int)Math.Floor(PanelSize.Width * 0.95);
            Mod_Icon[EntryID] = new PictureBox();
            Mod_Icon[EntryID].Size = new System.Drawing.Size(IconSize, (int)Math.Floor(IconSize * 0.9));
            Mod_Icon[EntryID].Location = new Point(PanelSize.Width / 2 - IconSize / 2, 2);
            ...
            SizeF LabelSize = g.MeasureString(Mod_NameLabel[EntryID].Text, Mod_NameLabel[EntryID].Font);
            ...
            Mod_NameLabel[EntryID].Location = new Point(Mod_Icon[EntryID].Left, Mod_Icon[EntryID].Bottom);
            Mod_NameLabel[EntryID].Size = new Size(Mod_Icon[EntryID].Width,(int)Math.Floor(LabelSize.Height));
            ...
            Mod_AuthorLabel[EntryID].Size = new Size(Mod_Icon[EntryID].Width, (int)Math.Floor(LabelSize.Height));
            Mod_AuthorLabel[EntryID].Location = new Point(Mod_Icon[EntryID].Left, Mod_NameLabel[EntryID].Bottom);
            ...
            LabelSize = g.MeasureString(Mod_VersionLabel[EntryID].Text, Mod_VersionLabel[EntryID].Font);
            Mod_VersionLabel[EntryID].Location = new Point((int)Math.Floor(PanelSize.Width - LabelSize.Width * 1.25), Mod_AuthorLabel[EntryID].Bottom);
            ...
            Mod_DLIcon[EntryID].Size = new System.Drawing.Size(PanelSize.Height/10, PanelSize.Height / 10);
            Mod_DLIcon[EntryID].Location = new Point(Mod_Icon[EntryID].Left, Mod_AuthorLabel[EntryID].Bottom);
            ...
            LabelSize = g.MeasureString(Mod_DownloadsLabel[EntryID].Text, Mod_DownloadsLabel[EntryID].Font);
            Mod_DownloadsLabel[EntryID].Location = new Point(Mod_DLIcon[EntryID].Right, (int)Math.Floor(Mod_VersionLabel[EntryID].Top + LabelSize.Height *0.1));
        ...
        private async void DownloadMod_Button_Click(object sender, EventArgs e)
        {
            if (DownloadMod_Button.Text != "Mod Downloaded!")
            {
                ...
                await DownloadFile(DownloadMod_Button.Tag.ToString(), ModName_Label.Text);
            }
        }

        async Task DownloadFile(string link, string ModName)
        {
            try
            {
                ...
                string targetPath = Application.StartupPath + @"Mods\" + LoadedWLVersion + @"\Downloads\" + ModName + ".wlmm";
                ...
                {
                    http.DefaultRequestHeaders.UserAgent.ParseAdd("WLMM");
                    using (var response = await http.GetAsync(fileLink, HttpCompletionOption.ResponseHeadersRead))
                    {
                        response.EnsureSuccessStatusCode();
                        ...
                        using (var fileStream = new FileStream(targetPath, FileMode.Create, FileAccess.Write, FileShare.None, 81920, true))
                        {
                            ...
                            while ((read = await contentStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
                            {
                                ...
                                double percentage = contentLength > 0 ? (totalRead * 100.0 / contentLength) : 0.0;
                                DownloadProgress_PB.Value = (int)Math.Truncate(Math.Min(100, percentage));
                                ...
                                if (timeElapsed.TotalMilliseconds > 250)
                                {
                                    ...
                                    double speedInBytesPerSecond = bytesSinceLastUpdate / Math.Max(1.0, timeElapsed.TotalSeconds);

                                    if (speedInBytesPerSecond < 0)
                                        speedInBytesPerSecond = 0;

                                    string DLSpeed = FormatBytes(speedInBytesPerSecond) + "/s";
                                    string DLCurrent = string.Format("{0} / {1}", FormatBytes(totalRead), contentLength > 0 ? FormatBytes(contentLength) : "?");
                                    string DLTimeLeft = string.Empty;

                                    if (contentLength > 0 && totalRead > 0 && totalRead < contentLength)
                                    {
                                ...
                                ProgressInfo_Label.Text = "Downloading... " + (int)Math.Truncate(Math.Min(100, percentage)) + "%";
